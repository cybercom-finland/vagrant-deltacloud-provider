  ...
  describe 'call' do
    context 'when server is active' do
      it 'become shutoff after one retry' do
        deltacloud.stub(:get_instance_details).and_return({ 'status' => 'ACTIVE' }, { 'status' => 'STOPPED' })
      ...
      it 'timeout after one retry' do
        deltacloud.stub(:get_instance_details).and_return({ 'status' => 'ACTIVE' }, { 'status' => 'ACTIVE' })
